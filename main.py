...
comm_lib=windll.LoadLibrary("plcommpro.dll")
print('\n', repr(comm_lib))

###connection parametres
dev_str=create_string_buffer(b"protocol=TCP, ipaddress=192.168.9.41, port=4370,t imeout=2000, passwd=") #controller address
print('\n', repr(dev_str))
...
info_buffer = create_string_buffer(2048)
print('\nBuffer created')		
info_items = create_string_buffer(b"DeviceID, Door1SensorType, Door1Drivertime, Door1Intertime") #dev_data
...
while 1:
    rt_log = create_string_buffer(256) #creating a buffer, why I don't know
    ...
    ###if len(log_card) > 1: #we've got positive event
        ...
        post_data['timeOf'] = log_time
        post_data['plate'] = log_card
        post_data['hash'] = code.hexdigest()
